import { useDispatch, useSelector } from "react-redux"
import todoSlice from "../redux/todoSlice"



const List=()=>{
    const list=useSelector((reduce)=>reduce)
    const dispatch=useDispatch()
    const handleCompleted=(todo)=>{
        console.log(list);
        dispatch(todoSlice.actions.completeTodo(todo))
    }
    return (
        <div className="list">
            <ul>
                {list.map(el=>(
                <li key={el.id}>
                    {el.isCompleted?
                    <svg onClick={()=>{handleCompleted(el)}} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgba(0,173,136,1)">
                        <path d="M12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22ZM12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20Z"></path>
                    </svg>:
                    <svg onClick={()=>{handleCompleted(el)}} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgba(0,173,136,1)">
                        <path d="M12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22ZM11.0026 16L18.0737 8.92893L16.6595 7.51472L11.0026 13.1716L8.17421 10.3431L6.75999 11.7574L11.0026 16Z"></path>
                    </svg>}
                    {el.todo}
                </li>
                ))}
            </ul>
        </div>
    )
}
export default List